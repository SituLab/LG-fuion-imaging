...
% Suppress undesired warnings on client (and workers if required). Destruction of this Constant
% will trigger the warning state to be reverted.
warningCleanupObject = suppressWarningsOnAll('MATLAB:rankDeficientMatrix', usePool);
...
if dispnum>1 % 'final' or 'iter' 
    if usePool 
        workerIdx = internal.stats.parallel.statParallelStore('workerID');
...
for j=1:maxiter
    ...
    if dispnum>2 % 'iter'
       if usePool 
           fprintf(dispfmt,workerIdx,repnum,j,dnorm,delta);
       else
...
% ---------------------------
function C = suppressWarningsOnAll(warningID, usePool)
C = parallel.pool.Constant(@() warning('off', warningID), @warning);
% Evaluate on client.
warningState = C.Value;
% Evaluate on workers if required.
if usePool
    f = parfevalOnAll(@(x) x.Value, 1, C);
    warningStateOnWorkers = fetchOutputs(f);
end 
end % suppressWarningsOnAll
