function [Reached_Pattern,Global_image,ff] = mergeimages(Maximum_intensity,O,EstimatedNumber,xx,yy,xpixel,ypixel,num)
...
Central_Xposition = zeros(EstimatedNumber,1); 
%  Create the empty matrix to store the position of each emitter in X coordinate
Central_Yposition = zeros(EstimatedNumber,1); 
%  Create the empty matrix to store the position of each emitter in Y coordinate
ff = 0;
    ...
    if k == 1
        ...
        for zz = 1:1
            ...
            global_image(Central_Xposition(Reached_Pattern(k))+(-floor(xpixel/2):floor(xpixel/2)-1),...
                ...
                = O{Reached_Pattern(k)};
            Global_image = Global_image + global_image;
            ff = ff+1;
        end
