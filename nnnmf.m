function [wbest,hbest,normbest,jbest] = nnnmf(a,k,varargin)
%% Adapted version based on nnmf.m
...
% ---- Nested functions ----
    ...
    function cellout = loopBody(iter,S)
        ...
        whtry = cell(5,1); % whtry{1} = norm of error
                           % whtry{3} = w
                           % whtry{4} = h
        ...
        [whtry{3},whtry{4},whtry{1},whtry{5}] = ...
...
% -------------------
function [w,h,dnorm,j] = nnmf1(a,w0,h0,ismult,maxiter,tolfun,tolx,...
