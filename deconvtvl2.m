...
% solving the following TV minimization problem
%
% Input:      g      - the observed image
...
%     opts.beta      - regularization parameter
...
H = cast(H,'double');
ga = repmat(g,2);
ga = circshift(ga,[size(g,1)/2, size(g,2)/2]);
HtgA = conv2(ga(2:end,2:end),H,'valid');
...
for itr=1:max_itr
    % solve f-subproblem
    f_old = f;
    rhs   = fftn((mu/rho)*HtgA + Dt(u1-(1/rho)*y1,  u2-(1/rho)*y2, u3-(1/rho)*y3));
